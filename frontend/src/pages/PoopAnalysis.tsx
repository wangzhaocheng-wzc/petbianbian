        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-xl font-semibold text-gray-900">{t('analysis.header.title')}</h1>
            </div>
            ...
            <div className="flex items-center space-x-4">
              <TouchButton
                ...
                icon={TrendingUp}
              >
                <span className={isMobile ? 'hidden xs:inline' : ''}>{t('analysis.header.viewRecords')}</span>
              </TouchButton>
            </div>
          </div>
        </div>
          ...
          <div className="lg:col-span-2">
            ...
            {pets.length > 1 && (
              <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6 mb-4 sm:mb-6">
                <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('analysis.header.selectPet')}</h2>
            ...
            {showResults && currentAnalysis ? (
              <div className="mb-6">
                ...
                <div className="mb-6">
                  <SaveShareActions
                    record={currentAnalysis}
                    petName={pets.find(p => p.id === selectedPetId)?.name || t('common.pet')}
                    isNew={true}
                ...
                <div className="mb-4">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-lg font-semibold text-gray-900">{t('analysis.detail.title')}</h2>
                    <button
              ...
              <>
                ...
                {selectedPetId ? (
                  ...
                  <div className="bg-white rounded-lg shadow-sm p-6 sm:p-8 text-center">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">{t('analysis.header.pleaseSelectPet')}</h3>
                    <p className="text-sm sm:text-base text-gray-600 mb-4">
                      {pets.length === 0 
                        ? t('analysis.header.noPetsTip')
                        : t('analysis.header.selectPetTip')
                      }
                    </p>
                    {pets.length === 0 && (
                      <TouchButton
          ...
          <div className="space-y-4 lg:space-y-6">
            ...
            {recentAnalyses.length > 0 && (
              <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('recent.title')}</h3>
            ...
            {/* 健康提示 */}
            <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('tips.title')}</h3>
              <div className="space-y-3">
                <div className="flex items-start space-x-3">
                  <div className="w-2 h-2 bg-green-400 rounded-full mt-2 flex-shrink-0"></div>
                  <p className="text-sm text-gray-600">{t('tips.normal')}</p>
                </div>
                <div className="flex items-start space-x-3">
                  <div className="w-2 h-2 bg-yellow-400 rounded-full mt-2 flex-shrink-0"></div>
                  <p className="text-sm text-gray-600">{t('tips.change')}</p>
                </div>
                <div className="flex items-start space-x-3">
                  <div className="w-2 h-2 bg-red-400 rounded-full mt-2 flex-shrink-0"></div>
                  <p className="text-sm text-gray-600">{t('tips.consult')}</p>
                </div>
              </div>
            </div>
            ...
            {/* 便便形状参考 */}
            <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('shapeRef.title')}</h3>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">{t('shapeRef.type12')}</span>
                  <span className="text-red-600 font-medium">{t('shapeRef.hard')}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">{t('shapeRef.type34')}</span>
                  <span className="text-green-600 font-medium">{t('shapeRef.normal')}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-600">{t('shapeRef.type57')}</span>
                  <span className="text-yellow-600 font-medium">{t('shapeRef.soft')}</span>
                </div>
              </div>
            </div>
          </div>
