...
// 实例方法 - 获取健康状态描述
PoopRecordSchema.methods.getHealthStatusDescription = function(): string {
  const descriptions = {
    healthy: '健康 - 便便状态正常',
    warning: '警告 - 需要关注，建议调整',
    concerning: '异常 - 建议咨询兽医'
  };
  return descriptions[this.analysis.healthStatus as keyof typeof descriptions];
};

// 实例方法 - 获取形状描述
PoopRecordSchema.methods.getShapeDescription = function(): string {
  const descriptions = {
    type1: '第1型 - 硬球状（严重便秘）',
    type2: '第2型 - 块状（轻度便秘）',
  ...
  return descriptions[this.analysis.shape as keyof typeof descriptions];
};

// 静态方法 - 获取宠物的健康统计
PoopRecordSchema.statics.getHealthStatistics = async function(petId: string, days: number = 30) {
...
// 静态方法 - 获取健康趋势数据
PoopRecordSchema.statics.getHealthTrends = async function(petId: string, days: number = 30) {
  const startDate = new Date();
  startDate.setDate(startDate.getDate() - days);
...
export const PoopRecord = mongoose.model<IPoopRecord>('PoopRecord', PoopRecordSchema);