...
export class StatisticsService {
   ...
   * 计算健康趋势分析
   */
  static async calculateHealthTrends(
    petId: string, 
    days: number = 30,
    ...
    try {
      ...
      const results = await PoopRecord.aggregate(pipeline);

      // 填充缺失的日期/周期
      const trendPoints: HealthTrendPoint[] = [];
   ...
   * 计算周期统计数据
   */
  static async calculatePeriodStatistics(
    petId: string,
    period: 'week' | 'month' | 'quarter' | 'year'
    ...
    try {
      ...
      // 计算形状分布
      const shapeDistribution: { [key: string]: number } = {};
      result.shapes.forEach((shape: string) => {
        shapeDistribution[shape] = (shapeDistribution[shape] || 0) + 1;
      });

      // 计算症状频率
      const symptomsFrequency: { [key: string]: number } = {};
      result.symptoms.forEach((symptomArray: string[]) => {
   ...
   * 异常模式识别
   */
  static async detectAnomalyPatterns(
    petId: string,
   ...
   * 对比分析（当前周期 vs 上一周期）
   */
  static async performComparisonAnalysis(
    petId: string,
    ...
    try {
      ...
      const previousStats = await this.calculatePeriodStatisticsForDateRange(
        petId, 
        previousStartDate, 
        previousEndDate,
        period
  ...
  private static async calculatePeriodStatisticsForDateRange(
    petId: string,
    startDate: Date,
    endDate: Date,
    period: 'week' | 'month' | 'quarter' | 'year'
  ...
  private static detectSuddenHealthDecline(records: any[]): AnomalyPattern | null {
  ...
  private static detectPersistentWarning(records: any[]): AnomalyPattern | null {
  ...
  private static detectFrequencyDrop(records: any[], windowDays: number): AnomalyPattern | null {
  ...
  private static detectConfidenceDrop(records: any[]): AnomalyPattern | null {
    if (records.length < 5) return null;
  ...
  private static detectShapePatternChange(records: any[]): AnomalyPattern | null {
    if (records.length < 10) return null;
    ...
    // 计算形状分布
    const getShapeDistribution = (recordSet: any[]) => {
