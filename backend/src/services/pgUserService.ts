...
const mapUserAggregate = (row: any): UserAggregate => {
  return {
    id: row.id,
...
export const getUserByEmail = async (email: string): Promise<UserAggregate | null> => {
  const pool = await getPostgresPool();
  const r = await pool.query(
    `SELECT u.id, u.username, u.email, u.avatar_url, u.role, u.is_active, u.is_verified, u.created_at, u.updated_at, u.last_login_at,
...
export const getUserAuthByEmail = async (email: string): Promise<{ agg: UserAggregate; passwordHash: string } | null> => {
  const pool = await getPostgresPool();
  const r = await pool.query(
    `SELECT u.id, u.username, u.email, u.password_hash, u.avatar_url, u.role, u.is_active, u.is_verified, u.created_at, u.updated_at, u.last_login_at,
            p.first_name, p.last_name, p.phone, p.location, p.bio,
            pref.notifications, pref.email_updates, pref.language,
            s.total_analysis, s.total_posts, s.reputation
       FROM users u
...
export const getUserById = async (id: string): Promise<UserAggregate | null> => {
  const pool = await getPostgresPool();
  const r = await pool.query(
    `SELECT u.id, u.username, u.email, u.avatar_url, u.role, u.is_active, u.is_verified, u.created_at, u.updated_at, u.last_login_at,
            p.first_name, p.last_name, p.phone, p.location, p.bio,
...
export const createUser = async (params: { username: string; email: string; password: string }): Promise<UserAggregate> => {
  const pool = await getPostgresPool();
  ...
  const ures = await pool.query(
    `INSERT INTO users (username, email, password_hash, role, is_active, is_verified, created_at, updated_at)
     ...
     RETURNING id, username, email, avatar_url, role, is_active, is_verified, created_at, updated_at, last_login_at`,
  ...
  const agg = await getUserById(userId);
  if (!agg) throw new Error('创建用户失败');
  return agg;
};
...
export const updateLastLoginById = async (userId: string, date: Date): Promise<void> => {
  const pool = await getPostgresPool();
  await pool.query('UPDATE users SET last_login_at = $2, updated_at = now() WHERE id = $1', [userId, date]);
};

export const recordRefreshTokenById = async (userId: string, token: string, expiresAt: Date): Promise<void> => {
  const pool = await getPostgresPool();
