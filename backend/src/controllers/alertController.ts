...
import { AlertService, CreateAlertRuleRequest, UpdateAlertRuleRequest } from '../services/alertService';
...
export class AlertController {
   ...
   * 创建提醒规则
   */
  static async createAlertRule(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      // 验证输入
  ...
  static async getUserAlertRules(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const rules = await AlertService.getUserAlertRules(
        userId,
        petId as string,
   ...
   * 更新提醒规则
   */
  static async updateAlertRule(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      // 验证输入
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
      ...
      const rule = await AlertService.updateAlertRule(ruleId, updates);
   ...
   * 删除提醒规则
   */
  static async deleteAlertRule(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { ruleId } = req.params;
  ...
  static async triggerAlertCheck(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const triggerResults = await AlertService.checkAndTriggerAlerts(petId, userId);
   ...
   * 获取异常检测结果
   */
  static async getAnomalyDetection(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { petId } = req.params;
      ...
      const anomalies = await AnomalyDetectionService.detectAnomalies(
        petId,
        analysisWindow ? parseInt(analysisWindow as string) : undefined,
        baselineWindow ? parseInt(baselineWindow as string) : undefined
      );

      const summary = await AnomalyDetectionService.getAnomalyDetectionSummary(petId);
   ...
   * 获取健康模式分析
   */
  static async getHealthPattern(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { petId } = req.params;
      const { days } = req.query;

      const pattern = await AnomalyDetectionService.analyzeHealthPattern(
        petId,
  ...
  static async createDefaultRules(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      const count = await AlertService.createDefaultRulesForUser(userId);
   ...
   * 批量检查提醒（管理员功能）
   */
  static async batchCheckAlerts(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      // 这里应该添加管理员权限检查
      // if (!req.user?.isAdmin) { ... }

      const result = await AlertService.batchCheckAlerts();
   ...
   * 获取提醒规则模板
   */
  static async getAlertRuleTemplates(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const templates = [
        {
          id: 'health_decline',
