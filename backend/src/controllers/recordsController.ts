...
export class RecordsController {
  ...
  static async getRecords(req: Request, res: Response): Promise<void> {
    try {
      ...
      const validSortFieldsMap: Record<string, string> = {
      ...
      const whereClauses: string[] = ['pr.user_id = $1'];
      const values: any[] = [userId];
      let idx = 2;

      if (petId) { whereClauses.push(`pr.pet_id = $${idx}`); values.push(petId); idx++; }
      if (healthStatus) { whereClauses.push(`pr.health_status = $${idx}`); values.push(healthStatus); idx++; }
      if (shape) { whereClauses.push(`pr.shape = $${idx}`); values.push(shape); idx++; }
      if (startDate) { whereClauses.push(`pr.timestamp >= $${idx}`); values.push(new Date(startDate)); idx++; }
      if (endDate) { whereClauses.push(`pr.timestamp <= $${idx}`); values.push(new Date(endDate)); idx++; }
      if (minConfidence !== undefined) { whereClauses.push(`pr.confidence >= $${idx}`); values.push(Number(minConfidence)); idx++; }
      if (maxConfidence !== undefined) { whereClauses.push(`pr.confidence <= $${idx}`); values.push(Number(maxConfidence)); idx++; }

      const symptomsArray = Array.isArray(symptoms)
        ? symptoms
        : (typeof symptoms === 'string' ? symptoms.split(',').map((s: string) => s.trim()).filter(Boolean) : []);
      ...
      const formattedRecords = listRes.rows.map((row: any) => ({
        id: row.id,
        petId: row.pet_id,
        pet: {
          id: row.pet_id,
  ...
  static async updateRecord(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (userNotes !== undefined) { setClauses.push(`user_notes = $${idx}`); values.push(userNotes); idx++; }
      if (symptoms !== undefined) { setClauses.push(`symptoms = $${idx}::text[]`); values.push(Array.isArray(symptoms) ? symptoms : (symptoms == null ? [] : [String(symptoms)])); idx++; }
      if (isShared !== undefined) { const sharedFlag = typeof isShared === 'string' ? isShared === 'true' : !!isShared; setClauses.push(`is_shared = $${idx}`); values.push(sharedFlag); idx++; }
  ...
  static async getStatisticsOverview(req: Request, res: Response): Promise<void> {
    try {
      ...
      const daysMap = { week: 7, month: 30, quarter: 90, year: 365 } as const;
      const periodStr = typeof period === 'string'
        ? period
        : Array.isArray(period) && typeof period[0] === 'string'
          ? period[0]
          : 'month';
      const days = daysMap[periodStr as keyof typeof daysMap] ?? 30;
  ...
  static async getAggregationSummary(req: Request, res: Response): Promise<void> {
    try {
      ...
      const petSummaries = rows.map(r => {
        ...
        (r.shape_list || []).forEach((shape: string) => {
      ...
      res.json({
        ...
        data: {
          ...
          totalRecords: petSummaries.reduce((sum: number, pet: any) => sum + pet.totalRecords, 0)
        }
      });
  ...
  static async batchDeleteRecords(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { rowCount } = await pool.query(
        'DELETE FROM poop_records WHERE user_id = $1 AND id = ANY($2::uuid[])',
        [userId, recordIds]
      );

      Logger.info(`批量删除记录: ${rowCount}/${recordIds.length}`);
  ...
  static async batchUpdateRecords(req: Request, res: Response): Promise<void> {
    try {
      ...
      const setClauses: string[] = [];
      const values: any[] = [userId, recordIds];
      let idx = 3;
      ...
      Logger.info(`批量更新记录: ${result.rowCount}/${recordIds.length}`);
      res.json({
        success: true,
