...
async function resolveMongoObjectId(id: string, entity: 'user' | 'pet'): Promise<mongoose.Types.ObjectId> {
...
export const getPosts = async (req: Request, res: Response) => {
  try {
    ...
    if (dbPrimary === 'postgres') {
      ...
      // 构建筛选条件
      const whereClauses: string[] = ["p.status = 'published'", "p.moderation_status = 'approved'"];
      const params: any[] = [];
      let paramIndex = 1;
      ...
      if (tags && (Array.isArray(tags) ? (tags as string[]).length : 1)) {
        const tagsArray = Array.isArray(tags) ? (tags as string[]) : [String(tags)];
    ...
    categoryStats.forEach(stat => {
      ...
      if (category) category.count = stat.count;
    });
...
export const createPost = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (dbPrimary === 'postgres') {
      ...
      // 验证宠物所有权（如果提供）
      let petIdForPg: string | null = null;
      if (petId) {
    ...
    let mongoUserId: mongoose.Types.ObjectId;
    try {
    ...
    let mongoPetId: mongoose.Types.ObjectId | undefined;
    if (petId) {
      try {
...
export const updatePost = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (dbPrimary === 'postgres') {
      ...
      try {
        ...
        if (r1.rows[0]?.id) {
          ...
          const r2 = await pool.query('SELECT id FROM users WHERE external_id = $1 LIMIT 1', [String(userId)]);
          if (r2.rows[0]?.id) userIdPg = r2.rows[0].id;
        }
      } catch (e) {
        console.error('用户ID映射失败:', e);
      }
      ...
      try {
        ...
        const values: any[] = [];
        let idx = 1;
        if (title !== undefined) { sets.push(`title = $${idx}`); values.push(String(title).trim()); idx++; }
        if (content !== undefined) { sets.push(`content = $${idx}`); values.push(String(content).trim()); idx++; }
        if (category !== undefined) { sets.push(`category = $${idx}`); values.push(category); idx++; }
        if (isAnonymous !== undefined) { sets.push(`is_anonymous = $${idx}`); values.push(Boolean(isAnonymous)); idx++; }
        if (sets.length > 0) {
        ...
        if (images !== undefined) {
          ...
          if (arr.length > 0) {
            const vals: any[] = [];
            const placeholders: string[] = [];
            arr.forEach((url: string, i: number) => {
    ...
    let mongoUserId: mongoose.Types.ObjectId;
    try { mongoUserId = await resolveMongoObjectId(userId, 'user'); } catch { return res.status(401).json({ success: false, message: '用户映射未建立，请重新登录后重试' }); }
    ...
    if (tags !== undefined) { post.tags = Array.isArray(tags) ? tags.map((tag: string) => tag.trim()).filter(Boolean) : []; }
    if (category !== undefined) {
      const validCategories = ['health', 'help', 'experience', 'general'];
      if (!validCategories.includes(category)) { return res.status(400).json({ success: false, message: '无效的分类' }); }
  ...
  try {
    ...
    if (dbPrimary === 'postgres') {
      ...
      const postRes = await pool.query(
        'SELECT id, user_id, status::text AS status FROM community_posts WHERE id = $1 LIMIT 1',
        [id]
      );
      const postRow = postRes.rows[0];
      if (!postRow) {
...
export const toggleLikePost = async (req: AuthRequest, res: Response) => {
  try {
    ...
    try { userObjectId = await resolveMongoObjectId(userId, 'user'); } catch { return res.status(401).json({ success: false, message: '用户映射未建立，请重新登录后重试' }); }
    const likes = post.interactions.likes;
    const userIndex = likes.findIndex((uid: mongoose.Types.ObjectId) => uid.toString() === userObjectId.toString());
    if (userIndex > -1) { likes.splice(userIndex, 1); } else { likes.push(userObjectId); }
    await post.save();
  ...
  try {
    ...
    if (dbPrimary === 'postgres') {
      ...
      const countRes = await pool.query(
        `SELECT COUNT(*)::int AS total
         FROM comments
         WHERE post_id = $1 AND is_deleted = false AND moderation_status = 'approved' AND parent_id IS NULL`,
        [id]
      );
      const totalItems: number = countRes.rows[0]?.total || 0;
      ...
      const topIds = topRows.map((r: any) => r.id);
      ...
      const comments = topRows.map((r: any) => ({
        id: String(r.id),
        postId: String(r.post_id),
        userId: String(r.user_id),
        content: r.content,
...
export const createComment = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (dbPrimary === 'postgres') {
      ...
      // 校验帖子存在与可评论
      const postRes = await pool.query(
        `SELECT status::text AS status, moderation_status::text AS moderation_status FROM community_posts WHERE id = $1 LIMIT 1`,
        [id]
      );
      const postRow = postRes.rows[0];
      if (!postRow) {
      ...
      // 父评论校验（如果是回复）
      let parentIdPg: string | null = null;
      if (parentId) {
        const parentRes = await pool.query(
          `SELECT id, post_id FROM comments WHERE id = $1 LIMIT 1`,
          [String(parentId)]
        );
      ...
      const userRes = await pool.query(
        `SELECT u.id AS user_id_for_user, u.username AS user_username, u.avatar_url AS user_avatar_url,
                up.first_name AS user_first_name, up.last_name AS user_last_name
    ...
    await comment.save();
    await CommunityPost.findByIdAndUpdate(id, { $push: { comments: comment._id } });
  ...
  try {
    ...
    if (dbPrimary === 'postgres') {
      ...
      const cmtRes = await pool.query(
        `SELECT id, is_deleted, moderation_status::text AS moderation_status FROM comments WHERE id = $1 LIMIT 1`,
        [commentId]
      );
      ...
      let userIdPg: string | null = null;
      const r1 = await pool.query('SELECT id FROM users WHERE id = $1 LIMIT 1', [String(userId)]);
      if (r1.rows[0]?.id) { userIdPg = r1.rows[0].id; } else {
        const r2 = await pool.query('SELECT id FROM users WHERE external_id = $1 LIMIT 1', [String(userId)]);
        if (r2.rows[0]?.id) userIdPg = r2.rows[0].id;
      }
      if (!userIdPg) { return res.status(401).json({ success: false, message: '用户未认证或映射失败' }); }
    ...
    // Mongo 回退
    if (!mongoose.Types.ObjectId.isValid(commentId)) { return res.status(400).json({ success: false, message: '无效的评论ID' }); }
    const comment = await Comment.findById(commentId);
    if (!comment) { return res.status(404).json({ success: false, message: '评论不存在' }); }
    if (comment.isDeleted || comment.moderationStatus !== 'approved') { return res.status(403).json({ success: false, message: '评论不可访问' }); }
    let userObjectId: mongoose.Types.ObjectId;
    try { userObjectId = await resolveMongoObjectId(userId, 'user'); } catch { return res.status(401).json({ success: false, message: '用户映射未建立，请重新登录后重试' }); }
...
export const deleteComment = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (dbPrimary === 'postgres') {
      ...
      // 校验评论存在
      const cmtRes = await pool.query(
        'SELECT id, user_id, is_deleted FROM comments WHERE id = $1 LIMIT 1',
        [commentId]
      );
      const c = cmtRes.rows[0];
      if (!c) {
