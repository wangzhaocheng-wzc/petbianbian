...
// 实例方法 - 标记应用内通知已发送
NotificationSchema.methods.markInAppSent = async function () {
    this.channels.inApp.sent = true;
    this.channels.inApp.sentAt = new Date();
    await this.save();
};
// 实例方法 - 标记邮件已发送
NotificationSchema.methods.markEmailSent = async function (emailAddress) {
...
// 实例方法 - 标记推送已发送
NotificationSchema.methods.markPushSent = async function (deviceTokens) {
...
// 静态方法 - 获取待发送的通知
NotificationSchema.statics.getPendingNotifications = async function (limit = 100) {
    const now = new Date();
