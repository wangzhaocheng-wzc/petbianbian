...
class ComparisonService {
     ...
     * 获取多个宠物的对比分析数据
     */
    static async getMultiPetComparison(userId, petIds, days = 30) {
        if (petIds.length < 2) {
        ...
        const pets = await Pet_1.default.find({
            _id: { $in: petIds.map(id => new mongoose_1.default.Types.ObjectId(id)) },
     ...
     * 获取单个宠物的对比数据
     */
    static async getPetComparisonData(petId, startDate, endDate) {
        const petObjectId = new mongoose_1.default.Types.ObjectId(petId);
        // 获取统计数据
        const statisticsResult = await PoopRecord_1.PoopRecord.aggregate([
            {
                $match: {
                    petId: petObjectId,
        ...
        // 获取趋势数据
        const trendsResult = await PoopRecord_1.PoopRecord.aggregate([
            {
                $match: {
                    petId: petObjectId,
                    timestamp: { $gte: startDate, $lte: endDate }
        ...
        // 获取形状分布
        const shapeDistributionResult = await PoopRecord_1.PoopRecord.aggregate([
            {
                $match: {
                    petId: petObjectId,
                    timestamp: { $gte: startDate, $lte: endDate }
     ...
     * 生成对比分析结果
     */
    static generateComparisonAnalysis(pets, startDate, endDate, days) {
        const healthiestPet = pets.reduce((prev, current) => current.statistics.healthyPercentage > prev.statistics.healthyPercentage ? current : prev);
        const mostConcerningPet = pets.reduce((prev, current) => current.statistics.concerningPercentage > prev.statistics.concerningPercentage ? current : prev);
     ...
     * 生成建议信息
     */
    static generateRecommendations(pets) {
     ...
     * 获取宠物健康趋势对比
     */
    static async getPetHealthTrends(userId, petIds, days = 30) {
        if (petIds.length < 2) {
        ...
        // 获取所有宠物的趋势数据
        const allTrends = await PoopRecord_1.PoopRecord.aggregate([
            {
                $match: {
        ...
        // 计算健康趋势
        let averageHealthTrend = 'stable';
        if (trends.length >= 7) {
            const recentWeek = trends.slice(-7);
            const earlierWeek = trends.slice(-14, -7);
            if (recentWeek.length > 0 && earlierWeek.length > 0) {
                const recentAvg = recentWeek.reduce((sum, day) => {
                    const dayAvg = day.pets.reduce((petSum, pet) => petSum + pet.healthPercentage, 0) / day.pets.length;
                    return sum + dayAvg;
                }, 0) / recentWeek.length;
                const earlierAvg = earlierWeek.reduce((sum, day) => {
                    const dayAvg = day.pets.reduce((petSum, pet) => petSum + pet.healthPercentage, 0) / day.pets.length;
                    return sum + dayAvg;
