...
class AnomalyDetectionService {
     ...
     * 检测宠物健康异常
     */
    static async detectAnomalies(petId, analysisWindow = 14, // 分析窗口（天）
    baselineWindow = 30, // 基线窗口（天）
    rules) {
        try {
            logger_1.Logger.info(`开始异常检测: 宠物ID=${petId}, 分析窗口=${analysisWindow}天`);
            ...
            // 1. 频率异常检测
            const frequencyAnomaly = await this.detectFrequencyAnomaly(analysisRecords, baselineRecords, analysisWindow, detectionRules.frequencyThreshold);
            if (frequencyAnomaly.isAnomalous) {
                anomalies.push(frequencyAnomaly);
            }
            // 2. 健康状态恶化检测
            const healthDeclineAnomaly = await this.detectHealthDecline(analysisRecords, baselineRecords, detectionRules.healthDeclineThreshold);
            if (healthDeclineAnomaly.isAnomalous) {
                anomalies.push(healthDeclineAnomaly);
            }
            // 3. 模式变化检测
            const patternChangeAnomaly = await this.detectPatternChange(analysisRecords, baselineRecords, detectionRules.patternChangeThreshold);
            if (patternChangeAnomaly.isAnomalous) {
                anomalies.push(patternChangeAnomaly);
            }
            // 4. 一致性变化检测
            const consistencyAnomaly = await this.detectConsistencyChange(analysisRecords, baselineRecords);
            if (consistencyAnomaly.isAnomalous) {
                anomalies.push(consistencyAnomaly);
            }
            logger_1.Logger.info(`异常检测完成: 发现${anomalies.length}个异常`);
            return anomalies;
        }
        catch (error) {
     ...
     * 检测频率异常
     */
    static async detectFrequencyAnomaly(analysisRecords, baselineRecords, analysisWindow, threshold) {
     ...
     * 检测健康状态恶化
     */
    static async detectHealthDecline(analysisRecords, baselineRecords, threshold) {
     ...
     * 检测模式变化
     */
    static async detectPatternChange(analysisRecords, baselineRecords, threshold) {
     ...
     * 检测一致性变化
     */
    static async detectConsistencyChange(analysisRecords, baselineRecords) {
     ...
     * 获取形状分布
     */
    static getShapeDistribution(records) {
        const distribution = {};
     ...
     * 计算模式相似度
     */
    static calculatePatternSimilarity(current, baseline) {
     ...
     * 分析健康模式
     */
    static async analyzeHealthPattern(petId, days = 30) {
        try {
     ...
     * 获取异常检测摘要
     */
    static async getAnomalyDetectionSummary(petId) {
        try {
            ...
            logger_1.Logger.error('获取异常检测摘要失败:', error);
...
// 默认检测规则
AnomalyDetectionService.DEFAULT_RULES = {
    frequencyThreshold: {
        minPerWeek: 3, // 每周最少3次
