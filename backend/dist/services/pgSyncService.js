...
exports.deletePoopRecord = exports.upsertPoopRecord = exports.deactivatePet = exports.upsertPet = exports.isTokenRevoked = exports.revokeToken = exports.recordRefreshToken = exports.updateLastLogin = exports.ensureUser = void 0;
const crypto_1 = __importDefault(require("crypto"));
const postgres_1 = require("../config/postgres");
const hashToken = (token) => crypto_1.default.createHash('sha256').update(token).digest('hex');
const getUserIdByExternal = async (pool, externalId) => {
    const r = await pool.query('SELECT id FROM users WHERE external_id = $1', [externalId]);
    return r.rows[0]?.id || null;
};
const getPetIdByExternal = async (pool, externalId) => {
    const r = await pool.query('SELECT id FROM pets WHERE external_id = $1', [externalId]);
    return r.rows[0]?.id || null;
};
const ensureUser = async (user, options) => {
    try {
        const pool = await (0, postgres_1.getPostgresPool)();
        const now = new Date();
        const ures = await pool.query(`INSERT INTO users (username,email,password_hash,avatar_url,role,is_active,is_verified,external_id,created_at,updated_at)
       ...
       VALUES ($1,$2,$3,$4,$5,$6)
       ON CONFLICT (user_id) DO UPDATE SET first_name = EXCLUDED.first_name, last_name = EXCLUDED.last_name, phone = EXCLUDED.phone, location = EXCLUDED.location, bio = EXCLUDED.bio`, [userId, user.profile?.firstName || null, user.profile?.lastName || null, user.profile?.phone || null, user.profile?.location || null, user.profile?.bio || null]);
        await pool.query(`INSERT INTO user_preferences (user_id, notifications, email_updates, language)
       VALUES ($1,$2,$3,$4)
       ON CONFLICT (user_id) DO UPDATE SET notifications = EXCLUDED.notifications, email_updates = EXCLUDED.email_updates, language = EXCLUDED.language`, [userId, user.preferences?.notifications !== false, user.preferences?.emailUpdates !== false, user.preferences?.language || 'zh-CN']);
        await pool.query(`INSERT INTO user_stats (user_id, total_analysis, total_posts, reputation, updated_at)
       VALUES ($1,$2,$3,$4, now())
       ON CONFLICT (user_id) DO UPDATE SET total_analysis = EXCLUDED.total_analysis, total_posts = EXCLUDED.total_posts, reputation = EXCLUDED.reputation, updated_at = now()`, [userId, user.stats?.totalAnalysis || 0, user.stats?.totalPosts || 0, user.stats?.reputation || 0]);
        if (options?.refreshToken && options.refreshExpiresAt) {
         ...
         VALUES ($1,$2,$3)
         ON CONFLICT (user_id, refresh_token_hash) DO NOTHING`, [userId, tokenHash, options.refreshExpiresAt]);
        }
    }
    catch (err) {
...
const updateLastLogin = async (userExternalId, date) => {
    try {
        const pool = await (0, postgres_1.getPostgresPool)();
...
const recordRefreshToken = async (userExternalId, token, expiresAt) => {
    try {
        const pool = await (0, postgres_1.getPostgresPool)();
...
const revokeToken = async (token, userExternalId) => {
    try {
        const pool = await (0, postgres_1.getPostgresPool)();
        const tokenHash = hashToken(token);
        let userId = null;
        if (userExternalId)
            userId = await getUserIdByExternal(pool, userExternalId);
...
const upsertPet = async (ownerExternalId, pet) => {
    try {
        const pool = await (0, postgres_1.getPostgresPool)();
...
const deactivatePet = async (petExternalId) => {
    try {
        const pool = await (0, postgres_1.getPostgresPool)();
...
exports.deactivatePet = deactivatePet;
const upsertPoopRecord = async (userExternalId, petExternalId, record) => {
    try {
        const pool = await (0, postgres_1.getPostgresPool)();
        const userId = await getUserIdByExternal(pool, userExternalId);
        const petId = await getPetIdByExternal(pool, petExternalId);
        if (!userId || !petId)
            return; // 未建立映射则跳过（符合“无需迁移旧数据”的策略）
        const now = new Date();
        ...
        const detectedFeatures = record.analysis?.detectedFeatures
            ...
            : null;
        await pool.query(`INSERT INTO poop_records (
...
exports.upsertPoopRecord = upsertPoopRecord;
const deletePoopRecord = async (recordExternalId) => {
    try {
        const pool = await (0, postgres_1.getPostgresPool)();
...
exports.deletePoopRecord = deletePoopRecord;
//# sourceMappingURL=pgSyncService.js.map