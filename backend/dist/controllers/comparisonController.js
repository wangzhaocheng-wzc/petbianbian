 ...
 * 获取多宠物对比分析
 */
const getMultiPetComparison = async (req, res) => {
    try {
        ...
        const comparisonData = await comparisonService_1.ComparisonService.getMultiPetComparison(userId, petIdArray, daysNumber);
        res.json({
            success: true,
            message: '获取对比分析成功',
            data: comparisonData
...
exports.getMultiPetComparison = getMultiPetComparison;
/**
 * 获取宠物健康趋势对比
 */
const getPetHealthTrends = async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({
        ...
        const trendsData = await comparisonService_1.ComparisonService.getPetHealthTrends(userId, petIdArray, daysNumber);
        res.json({
            success: true,
            message: '获取健康趋势对比成功',
            data: trendsData
...
exports.getPetHealthTrends = getPetHealthTrends;
/**
 * 获取用户所有宠物的基本信息（用于选择对比宠物）
 */
const getPetsForComparison = async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({
...
exports.getPetsForComparison = getPetsForComparison;
