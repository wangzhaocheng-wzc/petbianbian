...
class RecordsController {
    ...
    static async getRecords(req, res) {
        try {
            ...
            const [records, totalCount] = await Promise.all([
                PoopRecord_1.PoopRecord.find(query)
                    .populate('petId', 'name type breed avatar')
                    .sort(sortOptions)
                    .skip(skip)
                    .limit(limitNum)
                    .lean(),
                PoopRecord_1.PoopRecord.countDocuments(query)
            ]);
            // 计算分页信息
    ...
    static async getRecordById(req, res) {
        try {
            ...
            const record = await PoopRecord_1.PoopRecord.findOne({ _id: id, userId })
    ...
    static async updateRecord(req, res) {
        try {
            ...
            const updatedRecord = await PoopRecord_1.PoopRecord.findOneAndUpdate({ _id: id, userId }, updateData, { new: true, runValidators: true }).populate('petId', 'name type breed avatar');
            if (!updatedRecord) {
                res.status(404).json({
                    success: false,
                    message: '记录不存在或无权限修改'
    ...
    static async deleteRecord(req, res) {
        try {
            ...
            const deletedRecord = await PoopRecord_1.PoopRecord.findOneAndDelete({ _id: id, userId });
            if (!deletedRecord) {
                res.status(404).json({
                    success: false,
                    message: '记录不存在或无权限删除'
    ...
    static async getAggregationSummary(req, res) {
        try {
            ...
            const petSummaries = await PoopRecord_1.PoopRecord.aggregate(pipeline);
            // 计算形状分布
            const processedSummaries = petSummaries.map(summary => {
                const shapeCount = {};
                summary.shapeDistribution.forEach((shape) => {
    ...
    static async batchDeleteRecords(req, res) {
        try {
            ...
            // 验证所有ID格式
            const validIds = recordIds.filter(id => mongoose_1.default.Types.ObjectId.isValid(id));
            ...
            logger_1.Logger.info(`批量删除记录: ${result.deletedCount}/${recordIds.length}`);
            res.json({
                success: true,
    ...
    static async batchUpdateRecords(req, res) {
        try {
            ...
            // 批量更新
            const result = await PoopRecord_1.PoopRecord.updateMany({
                _id: { $in: validIds },
                userId
            }, filteredUpdateData);
            logger_1.Logger.info(`批量更新记录: ${result.modifiedCount}/${recordIds.length}`);
            res.json({
                success: true,
