import { AIService, AIAnalysisResult, ProcessedImage } from '../../src/services/aiService';
...
describe('AIService', () => {
  describe('preprocessImage', () => {
    it('should preprocess test image successfully', async () => {
      ...
      const result = await AIService.preprocessImage(testImagePath);
    ...
    it('should throw error for invalid image path', async () => {
      ...
      await expect(AIService.preprocessImage(invalidPath)).rejects.toThrow('图片预处理失败');
    });
  });
  ...
  describe('validatePoopContent', () => {
    it('should validate test image content successfully', async () => {
      ...
      const result = await AIService.validatePoopContent(mockProcessedImage);
      
      expect(result).toBe(true);
    });

    it('should reject images that are too small', async () => {
      const smallImage: ProcessedImage = {
      ...
      const result = await AIService.validatePoopContent(smallImage);
      
      expect(result).toBe(false);
    });

    it('should handle content validation with random confidence', async () => {
      const normalImage: ProcessedImage = {
      ...
      const results = [];
      for (let i = 0; i < 10; i++) {
        const result = await AIService.validatePoopContent(normalImage);
        results.push(result);
      }
  ...
  describe('analyzePoopImage', () => {
    it('should analyze image and return valid result', async () => {
      ...
      const result = await AIService.analyzePoopImage(mockProcessedImage);
    ...
    it('should simulate analysis delay', async () => {
      ...
      await AIService.analyzePoopImage(mockProcessedImage);
  ...
  describe('validateAnalysisResult', () => {
    it('should validate correct analysis result', () => {
      ...
      const result = AIService.validateAnalysisResult(validResult);
      expect(result).toBe(true);
    });
    ...
    it('should reject result with missing required fields', () => {
      ...
      const result = AIService.validateAnalysisResult(invalidResult);
      expect(result).toBe(false);
    });

    it('should reject result with invalid confidence range', () => {
      const invalidResult: AIAnalysisResult = {
        shape: 'type4',
        healthStatus: 'healthy',
        confidence: 150, // 超出范围
        details: 'test',
    ...
    it('should reject result with invalid health status', () => {
      const invalidResult: AIAnalysisResult = {
        shape: 'type4',
        healthStatus: 'invalid' as any, // 无效状态
        confidence: 85,
        details: 'test',
        recommendations: ['test'],
        detectedFeatures: {
  ...
  describe('getShapeDescription', () => {
    it('should return correct descriptions for all shape types', () => {
      ...
      shapes.forEach(shape => {
        const description = AIService.getShapeDescription(shape);
    ...
    it('should return unknown type for invalid shape', () => {
      const description = AIService.getShapeDescription('invalid');
  ...
  describe('getHealthStatusDescription', () => {
    it('should return correct descriptions for all health statuses', () => {
      ...
      statuses.forEach((status, index) => {
        const description = AIService.getHealthStatusDescription(status);
    ...
    it('should return unknown status for invalid status', () => {
      const description = AIService.getHealthStatusDescription('invalid');
