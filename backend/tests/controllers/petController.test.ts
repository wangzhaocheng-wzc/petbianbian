...
describe('Pet Controller', () => {
  ...
  describe('GET /pets', () => {
    ...
    it('should get user pets successfully', async () => {
      ...
      expect(new Date(pets[0].createdAt).getTime()).toBeGreaterThanOrEqual(
        new Date(pets[1].createdAt).getTime()
      );
    });

    it('should return empty array for user with no pets', async () => {
      // 删除所有宠物
      await Pet.deleteMany({ ownerId: testUser._id });
  ...
  describe('POST /pets', () => {
    it('should create pet successfully', async () => {
      ...
      // 验证宠物已保存到数据库
      const savedPet = await Pet.findById(response.body.data._id);
  ...
  describe('DELETE /pets/:id', () => {
    ...
    it('should return error for already deleted pet', async () => {
      // 先删除宠物
      await Pet.findByIdAndUpdate(testPet._id, { isActive: false });
